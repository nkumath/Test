...
# word="word"
# temp=nltk.word_tokenize(str(word))
...
path="C:\\Users\\Eternally Ascend\\Desktop\\Problem_C_Data\\"+name+"2a.xlsx"
# path="C:\\Users\\Eternally Ascend\\Desktop\\Problem_C_Data\\2(a)MicrowareID01.xlsx"
...
for i in range(0,count):
    ...
    for y in x:
        ...
        if float(outputExcel[y,2])>MaxStar:
            MaxStar=float(outputExcel[y,2])
        if float(outputExcel[y,3])>MaxTitle:
            MaxTitle=float(outputExcel[y,3])
        if float(outputExcel[y,4])>MaxContent:
            MaxContent=float(outputExcel[y,4])
        if float(outputExcel[y,5])>MaxHelpfulness:
            MaxHelpfulness=float(outputExcel[y,5])
        if float(outputExcel[y,1])<MinPurchased:
            MinPurchased=float(outputExcel[y,1])
        if float(outputExcel[y,2])<MinStar:
            MinStar=float(outputExcel[y,2])
        if float(outputExcel[y,3])<MinTitle:
            MinTitle=float(outputExcel[y,3])
        if float(outputExcel[y,4])<MinContent:
            MinContent=float(outputExcel[y,4])
        if float(outputExcel[y,5])<MinHelpfulness:
            MinHelpfulness=float(outputExcel[y,5])
    ...
    for z in x:
        ...
        # PicoutputExcel[z,1]=outputExcel[z,1]/MaxPurchased
        # PicoutputExcel[z,2]=outputExcel[z,2]/MaxStar
        # PicoutputExcel[z,3]=outputExcel[z,3]/MaxTitle
        # PicoutputExcel[z,4]=outputExcel[z,4]/MaxContent
        # PicoutputExcel[z,5]=outputExcel[z,5]/MaxHelpfulness
    ...
    plt.ylabel("Trend")
    plt.plot(x,PicoutputExcel[:,1],label="Purchased")
    ...
    plt.savefig("./"+name+"/2/"+name+"TrendOf"+str(npdata[idList[i],3])+".png")
    plt.cla()
    plt.clf()
    ...
    writer = pd.ExcelWriter("./"+name+"/ForPicture"+name+"-"+str(i)+"-"+str(npdata[idList[i],3])+'-2a.xlsx')
    # writer = pd.ExcelWriter("./"+name+"/"+name+"-"+str(i)+"-"+str(npdata[idList[i],3])+'-2a.xlsx')
